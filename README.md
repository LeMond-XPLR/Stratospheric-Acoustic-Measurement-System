...
【概要】
  ...
  マイク（voicehat）から音声を取得しながら、パッシブブザーで定期的に既知周波数のトーンを
  鳴らし、チャンクごとにWAVファイルとして保存します。
  成層圏気球実験において、音響環境の記録と参照トーンの埋め込みを同時に行うことを目的と
  しています。
...
【設定パラメータ (スクリプト冒頭)】
  ...
  TOTAL_SECONDS   録音総時間 [秒]          デフォルト: 7200（2時間）
  SILENT_SECONDS  1サイクルの無音時間 [秒]  デフォルト: 6
  FREQ            ブザーで鳴らす周波数リスト デフォルト: [1000, 2000, 3000, 4000] Hz
  CYCLE_SECONDS   1サイクルの長さ           = SILENT_SECONDS + len(FREQ) = 10秒
  CHUNK_SECONDS   WAVファイル1個の長さ [秒]  デフォルト: 60（1分）
  BUZZER_PIN      GPIO ピン番号             デフォルト: 12
  DUTY            PWMデューティ比           デフォルト: 50000 (50%)
  SAVE_DIR        保存先ディレクトリ         /home/pi/kosen3-devkit/recording-mission
  FS              サンプリング周波数 [Hz]    デフォルト: 48000
...
【注意事項】
  ...
  - 保存先ディレクトリ (SAVE_DIR) は事前に作成しておいてください
    例: mkdir -p /home/pi/kosen3-devkit/recording-mission
...
【概要】
  recorder.py で録音・結合されたWAVファイルを読み込み、埋め込みトーン部分を
  FFT解析して各周波数のdB値をCSV形式で出力します。
...
【設定パラメータ (スクリプト冒頭)】
  ...
  WAV_PATH  入力WAVファイルのパス          デフォルト: "data_combined_13.wav"
  CYCLE     1サイクルの長さ [秒]           デフォルト: 10.0
  FREQS     解析対象の周波数リスト [Hz]     デフォルト: [1000, 2000, 3000, 4000]
  F_TOL     周波数トレランス（±Hz）        デフォルト: 80.0
  EDGE      トーン区間の端を除外する秒数    デフォルト: 0.05（立ち上がり・立ち下がり除去）

【解析の仕組み】
  WAVファイル全体をCYCLE秒ごとのブロックに分割し、各ブロック内の
  トーン区間（EDGE秒を除いた部分）に対してFFTを実行します。
  ...
  各区間にハニング窓をかけてrFFTを実行し、
  対象周波数 ± F_TOL Hz の範囲の平均振幅をdBに変換します。
...
【注意事項】
  ...
  - 録音チャンクを結合せずに単体で入力した場合も動作しますが、
    サイクル番号はそのファイル内での通し番号になります
...
================================================================================
  ...
  [Raspberry Pi 上]
    ...
    2. python3 recorder.py
       → chunk_YYYYMMDD_HHMMSS_000.wav
          chunk_YYYYMMDD_HHMMSS_001.wav
          ...
          chunk_YYYYMMDD_HHMMSS_119.wav
